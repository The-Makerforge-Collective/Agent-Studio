...
def update_workflow(wid: str, body: UpdateWorkflow,
...
def delete_workflow(wid: str, p: Principal = Depends(require_role("editor"))) -> dict[str, str]:
    with db.session() as s:
        w = _owned_workflow(s, wid, p)
        s.delete(w)
        s.commit()
...
@app.get("/api/v1/workflows/{wid}/code")
def workflow_code(wid: str, p: Principal = Depends(get_principal)) -> PlainTextResponse:
    ...
    with db.session() as s:
        ...
        return PlainTextResponse(code, media_type="text/x-python",
                                 headers={"Content-Disposition": f'attachment; filename="{w.name.replace(" ", "_").lower()}.py"'})


def _generate_langgraph_code(name: str, spec: Spec) -> str:
    ...
    lines = [
        '"""',
        f"LangGraph workflow: {name}",
        f"Generated by Agent Studio",
        '"""',
        "from __future__ import annotations",
        "",
    ...
    for n in spec.nodes:
        if n.type == "end":
            continue
        func_name = n.id.replace("-", "_")
        doc = NODE_DOCS.get(n.type, n.type)
        config_lines = json.dumps(n.config, indent=4) if n.config else "{}"
        lines.append(f"def {func_name}(state: State) -> State:")
        lines.append(f'    """{doc}"""')
        lines.append(f"    config = {config_lines}")
        if n.type == "agent":
            lines.append(f'    # TODO: call LLM with config["prompt"]')
            lines.append(f"    return state")
        elif n.type == "transform":
            lines.append(f'    # TODO: evaluate config["expr"]')
            lines.append(f"    return state")
        elif n.type == "cli":
            lines.append(f'    # TODO: run config["command"] in subprocess')
            lines.append(f"    return state")
        elif n.type == "router":
            lines.append(f'    # Routing handled by conditional_edges below')
            lines.append(f"    return state")
        elif n.type == "guardrail":
            lines.append(f'    # TODO: check config["blocked"] patterns')
            lines.append(f"    return state")
        else:
            lines.append(f"    # TODO: implement {n.type} logic")
            lines.append(f"    return state")
        lines.append("")
        lines.append("")

    lines.append("def build_graph() -> StateGraph:")
    lines.append(f'    """Build the {name} workflow graph."""')
    ...
    for n in spec.nodes:
        if n.type == "end":
            continue
        func_name = n.id.replace("-", "_")
        lines.append(f'    graph.add_node("{n.id}", {func_name})')

    lines.append("")

    if roots:
        lines.append(f'    graph.set_entry_point("{roots[0]}")')
    lines.append("")

    for n in spec.nodes:
        targets = adj.get(n.id, [])
        if n.type == "end" or not targets:
            continue
        if n.type == "router" and len(targets) == 2:
            lines.append(f"    graph.add_conditional_edges(")
            lines.append(f'        "{n.id}",')
            lines.append(f'        lambda s: "a" if s["data"].get("route") else "b",')
            t_map = {}
            for t in targets:
                if nodes_by_id.get(t, None) and nodes_by_id[t].type == "end":
                    t_map['"a"' if not t_map else '"b"'] = "END"
                else:
                    t_map['"a"' if not t_map else '"b"'] = f'"{t}"'
            lines.append(f"        {{{', '.join(f'{k}: {v}' for k, v in t_map.items())}}},")
            lines.append(f"    )")
        else:
            for t in targets:
                if nodes_by_id.get(t, None) and nodes_by_id[t].type == "end":
                    lines.append(f'    graph.add_edge("{n.id}", END)')
                else:
                    lines.append(f'    graph.add_edge("{n.id}", "{t}")')
    ...
    lines.append('    result = app.invoke({"messages": [], "data": {}})')
